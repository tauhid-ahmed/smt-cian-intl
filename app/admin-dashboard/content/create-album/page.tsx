...
import { useAddAlbumMutation } from "@/lib/api/albumApi";
...
export default function CreateNewAlbum() {
    ...
    const handlePublish = async () => {
        if (!albumTitle || !selectedArtist || !selectedGenre || !albumImageFile || !releaseDate) {
            toast.error("Please fill in all required fields (Title, Artist, Genre, Cover Image, Release Date)");
            return;
        }
        ...
        try {
            ...
            const response = await addAlbum(formData).unwrap();

            if (response.success) {
                toast.success(response.message || "Album published successfully!");
                setTimeout(() => {
                    router.back();
                }, 2000);
            } else {
                toast.error(response.message || "Failed to publish album");
            }
        } catch (error: any) {
            console.error("Failed to publish album:", error);
            toast.error(error?.data?.message || "An error occurred while publishing the album");
        }
    };
